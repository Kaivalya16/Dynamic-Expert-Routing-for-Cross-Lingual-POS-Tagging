...
# Create a reverse dictionary to map IDs back to human-readable tags
...
def main():
    ...
    # We want to track the routing weights specifically for each POS tag
    # Dictionary structure: {tag_id: list of weight tensors}
    tag_routing_history = {i: [] for i in range(NUM_LABELS)}

    # 4. Evaluation Loop
    with torch.no_grad(): # No gradients needed for testing
        for batch in tqdm(dataloader, desc="Evaluating Basque"):
            ...
            # Mask out the -100 padding/subword labels
            ...
            # Store routing weights for interpretability
            # Flatten arrays to iterate through valid tokens easily
    ...
    print(f"\n=== Zero-Shot Basque POS Accuracy: {accuracy * 100:.2f}% ===")

    print("\n=== Routing Interpretability by Linguistic Phenomenon ===")
    print("Which expert did the router trust for specific parts of speech?\n")
    
    # Analyze a few key phenomena
    ...
    for tag_name in phenomena_to_check:
        ...
        # Calculate the mean weight assigned to each expert for this specific POS tag
